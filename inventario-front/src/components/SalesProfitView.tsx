...
export function SalesProfitView() {
  ...
  const [isMonthSummaryOpen, setIsMonthSummaryOpen] = useState(true);

  // Estados para el Modal de Accionistas
  const [isShareholderModalOpen, setIsShareholderModalOpen] = useState(false);
  const [shareholderFinancials, setShareholderFinancials] = useState<any[]>([]);
  ...
  const handleOpenShareholderModal = async () => {
    ...
    try {
      const res = await api.get('/shareholders/financials');
  ...
  const profitByMonth = sales.reduce((acc: { [key: string]: { revenue: number; cost: number; profit: number } }, sale) => {
  ...
  const totalRevenue = filteredSales.reduce((acc, sale) => {
    const saleTotal = sale.items?.reduce((sum: number, item: any) => sum + calculateItemRevenue(item), 0) || 0;
    return acc + saleTotal;
  }, 0);

  const totalProfit = filteredSales.reduce((acc, sale) => {
    const saleProfit = sale.items?.reduce((sum: number, item: any) => sum + calculateItemProfit(item), 0) || 0;
  ...
  return (
    <div className="p-6 text-white max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg font-medium shadow transition flex items-center gap-2 text-sm"
          >
          ...
          <button 
            ...
            className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg font-medium shadow transition flex items-center gap-2 text-sm"
          >
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg overflow-hidden shadow-xl">
        <div 
          className="p-4 border-b border-gray-700 flex justify-between items-center font-semibold cursor-pointer hover:bg-gray-750 transition"
          onClick={() => setIsMonthSummaryOpen(!isMonthSummaryOpen)}
        >
          ...
          {selectedMonth !== 'todos' && (
            <button 
              onClick={(e) => { e.stopPropagation(); setSelectedMonth('todos'); }}
            ...
            <table className="w-full text-left border-collapse text-sm">
              ...
              <tbody>
                {filteredMonths.length > 0 ? (
                  filteredMonths.map(([month, data]: [string, any]) => (
                    <tr key={month} className="border-b border-gray-700/50 hover:bg-gray-750">
              ...
              {filteredSales.length > 0 ? (
                filteredSales.map((sale) => {
                  return sale.items?.map((item: any, idx: number) => {
                    ...
                    return (
                      <tr key={`${sale.id}-${idx}`} className="border-b border-gray-700/50 hover:bg-gray-750">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-gray-900 border border-gray-700 w-full max-w-md rounded-xl p-6 shadow-2xl text-white space-y-4">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
              <button
                onClick={() => { setSelectedMonth('todos'); setIsModalOpen(false); }}
                className={`w-full text-left px-4 py-2.5 rounded-lg border transition ${selectedMonth === 'todos' ? 'bg-indigo-600 border-indigo-500 font-semibold' : 'bg-gray-800 border-gray-700 hover:bg-gray-750'}`}
              >
                🌐 Mostrar Todos los Meses
              </button>

              {Object.keys(profitByMonth).map((month) => (
                <button
                  key={month}
                  onClick={() => { setSelectedMonth(month); setIsModalOpen(false); }}
                  className={`w-full text-left px-4 py-2.5 rounded-lg border transition ${selectedMonth === month ? 'bg-indigo-600 border-indigo-500 font-semibold' : 'bg-gray-800 border-gray-700 hover:bg-gray-750'}`}
                >
                  📅 {month}
                </button>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-gray-900 border border-gray-700 w-full max-w-6xl rounded-xl p-6 shadow-2xl text-white space-y-6">
            ...
            <div className="flex justify-end pt-2">
              <button
                onClick={() => setIsShareholderModalOpen(false)}
                className="bg-gray-700 hover:bg-gray-600 text-white px-5 py-2 rounded-lg text-sm transition font-medium"
              >
                Cerrar
              </button>
            </div>
