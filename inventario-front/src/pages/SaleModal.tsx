...
export function SaleModal({ onClose, onSaleSuccess }: { onClose: () => void, onSaleSuccess: () => void }) {
  const [products, setProducts] = useState<any[]>([]);
  const [shareholders, setShareholders] = useState<any[]>([]);
  const [shareholderProducts, setShareholderProducts] = useState<any[]>([]);
  const [customers, setCustomers] = useState<any[]>([]);
  const [warehouses, setWarehouses] = useState<any[]>([]);
  
  const [selectedItems, setSelectedItems] = useState<{ productId: string, shareholderId: string, quantity: number, unitPrice: number | '' }[]>([
    ...
    try {
      ...
      if (!finalClientId) {
        ...
        if (matchedCustomer) {
          ...
          const newCustRes = await api.post('/customers', { name: clientSearch.trim() });
      ...
      for (const item of selectedItems) {
        ...
        const sp = shareholderProducts.find(
          s => s.productId === item.productId && s.shareholderId === item.shareholderId
        );
      ...
      <div className="bg-gray-800 p-6 rounded-lg w-full max-w-4xl border border-gray-700 text-white shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div className="relative" ref={clientDropdownRef}>
            ...
            <input
              type="text"
              placeholder="Escribe 'Genérico' o nombre de cliente..."
              value={clientSearch}
              onChange={(e) => {
            ...
            {showClientDropdown && clientSearch.trim() !== '' && (
              <div className="absolute z-10 w-full mt-1 bg-gray-900 border border-gray-700 rounded-md shadow-lg max-h-48 overflow-y-auto">
                {filteredCustomers.length > 0 ? (
                  ...
                  <div className="p-2 text-sm text-emerald-400 bg-gray-850">
        ...
        <div className="space-y-3 mb-4 max-h-60 overflow-y-auto pr-1">
          {selectedItems.map((item, index) => {
            ...
            return (
              <div key={index} className="flex gap-2 items-center bg-gray-900/50 p-3 rounded border border-gray-700/50">
                ...
                <input 
                  ...
                  value={item.quantity}
                  placeholder="Cant"
                  className="w-16 bg-gray-900 border border-gray-700 p-2 rounded text-white text-sm text-center"
                  onChange={(e) => {
                    const newItems = [...selectedItems];
                ...
                <div className="flex flex-col gap-1">
                  <input 
                    ...
                    title="Déjalo vacío para usar el precio normal o escribe una rebaja/precio de costo"
                    className="w-28 bg-gray-900 border border-indigo-500/50 p-2 rounded text-white text-sm text-right"
                    onChange={(e) => {
                      const newItems = [...selectedItems];
