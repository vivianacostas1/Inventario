...
import { warehouseService } from '../api/warehouse.service.ts';
...
export const WarehousesPage = () => {
  ...
  const [currentWarehouse, setCurrentWarehouse] = useState<Partial<Warehouse>>({ name: '', location: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && currentWarehouse.id) {
        await warehouseService.update(currentWarehouse.id, payload);
      } else {
        await warehouseService.create(payload as Omit<Warehouse, 'id'>);
      }
    ...
    <div className="p-8 max-w-6xl mx-auto text-white">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 rounded font-semibold text-sm transition"
        >
          + Nuevo Almacén
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-700 text-sm">
              {warehouses.length === 0 ? (
                ...
                warehouses.map((wh) => (
                  <tr key={wh.id} className="hover:bg-gray-750 transition">
                    ...
                    <td className="p-4 text-center space-x-2">
                      ...
                      <button
                        ...
                        className="px-3 py-1 bg-red-600/80 hover:bg-red-600 rounded text-xs transition"
                      >
                        Eliminar
                      </button>
                    </td>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded text-white focus:outline-none focus:border-indigo-500 text-sm"
