        ...
        const fetchStats = async () => {
            try {
                ...
                const [productsRes, customersRes, suppliersRes] = await Promise.all([
                    axios_1.default.get('/products').catch(() => ({ data: [] })),
                    axios_1.default.get('/customers').catch(() => ({ data: [] })),
                    axios_1.default.get('/suppliers').catch(() => ({ data: [] })),
                ]);
                setStats({
    ...
    return (<div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-md">
          ...
          <react_router_dom_1.Link to="/products" className="text-xs text-indigo-400 hover:underline mt-3 inline-block">
            Ver inventario &rarr;
      ...
      <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          <react_router_dom_1.Link to="/products" className="p-4 bg-gray-900/50 hover:bg-gray-700/50 border border-gray-700 rounded-lg text-center transition flex flex-col items-center justify-center space-y-2">
            <span className="text-2xl">📦</span>
            <span className="text-sm font-medium text-gray-200">Productos</span>
          </react_router_dom_1.Link>

          {user?.role === 'ADMIN' && (<react_router_dom_1.Link to="/users" className="p-4 bg-gray-900/50 hover:bg-gray-700/50 border border-gray-700 rounded-lg text-center transition flex flex-col items-center justify-center space-y-2">
              <span className="text-2xl">👥</span>
