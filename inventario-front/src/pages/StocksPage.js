...
function StocksPage() {
    ...
    const fetchStocks = async () => {
        try {
            ...
            const res = await axios_1.default.get('/stocks');
            setStocks(res.data);
        }
        catch (error) {
      ...
      <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden shadow-lg">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-700 text-sm">
            {loading ? (<tr><td colSpan={4} className="text-center p-4 text-gray-400">Cargando inventario...</td></tr>) : stocks.length === 0 ? (<tr><td colSpan={4} className="text-center p-4 text-gray-400">No hay existencias registradas.</td></tr>) : (stocks.map((s) => (<tr key={s.id} className="hover:bg-gray-750 transition">
                  <td className="p-3 font-medium text-indigo-300">{s.product?.name || '---'}</td>
                  <td className="p-3 text-gray-300">{s.warehouse?.name || '---'}</td>
                  <td className="p-3 text-center">
                    <span className={`px-2 py-1 rounded font-bold ${s.quantity > 10 ? 'text-emerald-400' : 'text-rose-400'}`}>
                      {s.quantity}
                    </span>
                  </td>
