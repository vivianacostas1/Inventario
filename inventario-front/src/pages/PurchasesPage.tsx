...
export function PurchasesPage() {
  ...
  const handleItemChange = (index: number, field: keyof PurchaseItemForm, value: string | number) => {
    const newItems = [...items];
  ...
  const handleConfirmReceive = async (e: React.FormEvent) => {
    ...
    try {
      await api.patch(`/purchases/${selectedPurchaseId}/status`, {
        status: 'RECEIVED',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (items.length === 0) return alert("Debes agregar al menos un producto a la compra");
    ...
    <div className="p-6 text-white h-full overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
          ...
          className="bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded font-semibold text-sm transition"
        >
          + Nueva Compra
        </button>
      </div>
      ...
      <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden shadow-lg">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-700 text-sm">
            {loading ? (
              <tr><td colSpan={7} className="text-center p-4 text-gray-400">Cargando compras...</td></tr>
            ) : purchases.length === 0 ? (
              <tr><td colSpan={7} className="text-center p-4 text-gray-400">No hay compras registradas.</td></tr>
            ) : (
              purchases.map((p) => (
                <tr key={p.id} className="hover:bg-gray-750 transition">
                  ...
                  <td className="p-3">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${
                      p.status === 'RECEIVED' ? 'bg-emerald-900 text-emerald-300' :
                  ...
                  <td className="p-3 text-center space-x-2">
                    {p.status === 'PENDING' && (
                      <button 
                        ...
                        className="bg-emerald-600 hover:bg-emerald-500 text-white px-2.5 py-1 rounded text-xs font-semibold transition shadow"
                    ...
                    <button 
                      ...
                      className="bg-blue-600 hover:bg-blue-500 text-white px-2.5 py-1 rounded text-xs font-semibold transition shadow"
                      title="Editar Compra"
                    >
                      Editar
                    </button>
          ...
          <div className="bg-gray-800 p-6 rounded-lg w-full max-w-4xl border border-gray-700 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="mt-6 border-t border-gray-700 pt-4">
                ...
                {items.length === 0 && (
                  <p className="text-sm text-gray-500 py-2">No hay productos agregados.</p>
                )}

                <div className="space-y-2">
                  {items.map((item, index) => (
                    <div key={index} className="flex gap-2 items-end bg-gray-900 p-3 rounded border border-gray-750">
