        ...
        <div className="flex bg-gray-900 p-1 rounded-lg border border-gray-700">
          <button
            ...
            className={`px-4 py-1.5 rounded text-sm font-semibold transition ${
              filterType === 'ALL' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded text-sm font-semibold transition ${
              filterType === 'IN' ? 'bg-emerald-900 text-emerald-300' : 'text-gray-400 hover:text-white'
            }`}
          >
            Entradas (IN)
          </button>
          <button
            ...
            className={`px-4 py-1.5 rounded text-sm font-semibold transition ${
              filterType === 'OUT' ? 'bg-rose-900 text-rose-300' : 'text-gray-400 hover:text-white'
            }`}
          >
            Salidas (OUT)
          </button>
      ...
      <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden shadow-lg">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-700 text-sm">
            {loading ? (
              <tr><td colSpan={7} className="text-center p-4 text-gray-400">Cargando movimientos...</td></tr>
            ) : filteredMovements.length === 0 ? (
              <tr><td colSpan={7} className="text-center p-4 text-gray-400">No hay movimientos registrados para este filtro.</td></tr>
            ) : (
                ...
                <tr key={m.id} className="hover:bg-gray-750 transition">
                  ...
                  <td className="p-3">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${
                      m.type === 'IN' ? 'bg-emerald-900 text-emerald-300' : 'bg-rose-900 text-rose-300'
                    }`}>
                      {m.type}
                    </span>
                  </td>
