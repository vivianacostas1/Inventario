...
function AnalyticsPage() {
    ...
    if (loading)
        return <div className="p-6 text-white">Cargando análisis avanzado...</div>;
    if (!data)
        return <div className="p-6 text-white">No hay datos disponibles.</div>;
    ...
    const barChartData = {
        labels: salesPerMonth.map((m) => m.month),
        datasets: [{
                label: 'Ventas Totales (Bs)',
                data: salesPerMonth.map((m) => m.total),
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-gray-800 p-4 rounded-lg border border-gray-700 flex flex-col items-center">
          ...
          <div className="w-64 h-64 flex items-center justify-center">
            {topProducts.length > 0 ? <react_chartjs_2_1.Pie data={pieChartData}/> : <p className="text-sm text-gray-400">Sin ventas</p>}
          </div>
        </div>
        ...
        <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
          ...
          <div className="h-64 flex items-center justify-center">
            {salesPerMonth.length > 0 ? (<react_chartjs_2_1.Bar data={barChartData} options={{ responsive: true, maintainAspectRatio: false }}/>) : (<p className="text-sm text-gray-400">Sin registros mensuales</p>)}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {advancedInsights.map((item) => (<div key={item.productId} className={`p-2 rounded text-sm border-l-4 ${item.shouldNotBuy ? 'bg-amber-950/40 border-amber-500 text-amber-200' : 'bg-gray-900 border-emerald-500 text-gray-300'}`}>
        ...
        <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {alerts.length === 0 ? (<p className="text-sm text-gray-400">No hay alertas críticas de stock mínimo.</p>) : (alerts.map((a) => (<div key={a.id} className="flex justify-between items-center bg-gray-900 p-2 rounded text-sm border-l-4 border-red-500">
