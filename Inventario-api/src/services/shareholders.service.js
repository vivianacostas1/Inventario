...
exports.ShareholderService = void 0;
const prisma_1 = require("../config/prisma");
class ShareholderService {
    static async getAll() {
        const count = await prisma_1.prisma.shareholder.count({
            where: { isActive: true },
        });
        if (count === 0) {
            return [];
        }
        return await prisma_1.prisma.shareholder.findMany({
            where: { isActive: true },
            include: {
                purchases: {
                    include: {
    ...
    static async getById(id) {
        return await prisma_1.prisma.shareholder.findUnique({
            where: { id },
            include: {
                purchases: {
                    include: {
    ...
    static async create(data) {
        return await prisma_1.prisma.shareholder.create({
            data,
        });
    }
    static async update(id, data) {
        return await prisma_1.prisma.shareholder.update({
            where: { id },
            data,
        });
    }
    ...
    static async getFinancialSummary() {
        const shareholders = await prisma_1.prisma.shareholder.findMany({
            where: { isActive: true },
            include: {
                shareholderProducts: {
                    include: {
        ...
        const completedSaleItems = await prisma_1.prisma.saleItem.findMany({
            where: {
...
exports.ShareholderService = ShareholderService;
