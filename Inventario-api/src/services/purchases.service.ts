...
export class PurchaseService {
  ...
  static async create(data: CreatePurchaseDTO & { shareholderId?: string }) {
  ...
  static async updateStatus(id: string, status: PurchaseStatus, warehouseId?: string) {
    return await prisma.$transaction(async (tx) => {
      ...
      if (status === "RECEIVED" && purchase.status !== "RECEIVED") {
        ...
        if (!targetWarehouseId) {
          ...
          if (!defaultWarehouse) throw new Error("No hay almacenes configurados.");
